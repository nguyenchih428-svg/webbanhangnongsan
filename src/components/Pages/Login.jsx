...
const Login = () => {
  ...
  const handleForgotSubmit = async e => {
    ...
    try {
      ...
      setFpSuccess(data.message || "Đã đổi mật khẩu thành công. Bạn có thể đăng nhập ngay.");
      setFpNew("");
      setFpConfirm("");
    } catch (err) {
      const msg =
        err.response?.data?.error ||
        (err.code === "ERR_NETWORK" || err.response?.status === 404 ? "Chỉ hoạt động khi chạy npm run dev hoặc npm run preview." : null) ||
